...
database_name="hephaestus_artifact_rpc_${run_identity}_${run_attempt}_${RANDOM}"
...
test_output="${HEPHAESTUS_ARTIFACT_RPC_LOG:-${TMPDIR:-/tmp}/heph-artifact-rpc.${database_name}.log}"
...
if command -v psql >/dev/null 2>&1; then
  ...
  if [[ "${HEPHAESTUS_ARTIFACT_RPC_POSTGRES_MODE:-}" != container ]] || \
    [[ -z "${postgres_container}" ]] || ! command -v podman >/dev/null 2>&1; then
    printf 'artifact RPC deadline evidence requires host psql or explicit PostgreSQL container mode\n' >&2
    exit 1
  fi
  run_psql() {
...
cleanup() {
  ...
  printf 'artifact RPC deadline log: %s\n' "${test_output}"
  exit "${status}"
}
trap cleanup EXIT

mkdir -p -- "$(dirname -- "${test_output}")"
printf 'artifact RPC deadline disposable database: %s\n' "${database_name}"
...
HEPHAESTUS_POSTGRES_TEST_URL="${isolated_url}" \
  ...
  REAL_APP_ARTIFACT_RPC=1 \
  CARGO_INCREMENTAL="${CARGO_INCREMENTAL:-0}" \
  CARGO_BUILD_JOBS="${CARGO_BUILD_JOBS:-2}" \
  timeout --kill-after=30s "${HEPHAESTUS_ARTIFACT_RPC_TIMEOUT:-300s}" cargo +1.88.0 test \
    --manifest-path "${repo_root}/Cargo.toml" \
    --package hephaestus-app \
    --test artifact_deadline_cancellation \
    --features test-fixtures \
    -- --nocapture --test-threads=1 2>&1 | tee "${test_output}"

if ! grep -Fq 'REAL_APP_ARTIFACT_RPC_COMPLETED=1' "${test_output}"; then
  printf 'artifact RPC deadline evidence did not report a real completion marker; log: %s\n' \
    "${test_output}" >&2
  exit 1
fi
printf 'artifact RPC deadline evidence passed; log: %s\n' "${test_output}"

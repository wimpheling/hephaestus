...
SUMMARIZER = load("diagnostic_summarizer", ROOT / "summarize-cooking-diagnostics.py")
...
class BrowserSummaryTests(unittest.TestCase):
    ...
    def test_session_chat_requires_initial_recovery_concurrency_and_fork_journeys(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            fork = root / "browser.fork"
            fork.mkdir()
            (fork / "playwright.log").write_text(
                safe_session_report(
                    test_id=PROJECTOR.SESSION_CHAT_FORK_TEST_ID,
                    stages=PROJECTOR.SESSION_CHAT_FORK_STAGES,
                ),
                encoding="utf-8",
            )
            summary = PROJECTOR.project(root, "session-chat")
            ...
            self.assertEqual(summary["observed_phases"], ["initial", "recovery", "concurrency", "fork"])
            self.assertEqual(summary["passed_phases"], ["initial", "recovery", "concurrency", "fork"])
            self.assertEqual(summary["counts"], {"passed": 4, "failed": 0, "skipped": 0, "timed_out": 0})
            ...
            self.assertEqual(projected["observed_phases"], ["initial", "recovery", "concurrency", "fork"])
            bundle = root / "bundle"
            self.assertEqual(
            ...
            self.assertEqual(retained["observed_phases"], ["initial", "recovery", "concurrency", "fork"])
            triage = SUMMARIZER._project_browser_summary(
                root,
                [{"label": "browser-summary", "path": "summary.json"}],
            )
            self.assertEqual(triage["status"], "passed")
            self.assertEqual(triage["report_state"], "complete")
            self.assertEqual(triage["observed_phases"], ["initial", "recovery", "concurrency", "fork"])
            self.assertEqual(triage["passed_phases"], ["initial", "recovery", "concurrency", "fork"])
    ...
    def test_session_chat_failed_concurrency_remains_typed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for phase, test_id, stages in (
                ...
                ("fork", PROJECTOR.SESSION_CHAT_FORK_TEST_ID, PROJECTOR.SESSION_CHAT_FORK_STAGES),
            ):
            ...
            self.assertEqual(summary["report_state"], "complete")
            self.assertEqual(summary["observed_phases"], ["initial", "recovery", "concurrency", "fork"])
            self.assertEqual(summary["passed_phases"], ["initial", "recovery", "fork"])
            self.assertEqual(summary["counts"], {"passed": 3, "failed": 1, "skipped": 0, "timed_out": 0})

    def test_session_chat_failed_fork_remains_typed(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            for phase, test_id, stages in (
                ("initial", PROJECTOR.SESSION_CHAT_TEST_ID, PROJECTOR.SESSION_CHAT_STAGES),
                ...
                (browser / "playwright.log").write_text(
                    safe_session_report(
                        test_id=test_id,
                        stages=stages,
                        test_status="failed" if phase == "fork" else "passed",
                        failed_stage="session_chat_fork_response" if phase == "fork" else None,
                    ),
                    encoding="utf-8",
                )
            summary = PROJECTOR.project(root, "session-chat")

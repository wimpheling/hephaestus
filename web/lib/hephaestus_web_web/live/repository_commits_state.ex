defmodule HephaestusWebWeb.RepositoryCommitsState do
  @moduledoc "C2 state/effects contract for repository commit browsing."
  ...
  def reduce(state, event), do: RepositoryRouteModel.reduce(state, event, :commits)

  def execute(state, {:load, identity, generation}),
    do: RepositoryRouteModel.execute(state, identity, generation, :commits)

  def execute(effect, identity), do: RepositoryRouteModel.execute(effect, identity)

  def present(state), do: RepositoryRouteModel.present(state, :commits)
end

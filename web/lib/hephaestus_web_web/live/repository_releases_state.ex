defmodule HephaestusWebWeb.RepositoryReleasesState do
  @moduledoc "C2 state/effects contract for repository release browsing."
  ...
  def reduce(state, event), do: RepositoryRouteModel.reduce(state, event, :releases)

  def execute(state, {:load, identity, generation}),
    do: RepositoryRouteModel.execute(state, identity, generation, :releases)

  def execute(effect, identity), do: RepositoryRouteModel.execute(effect, identity)

  def present(state), do: RepositoryRouteModel.present(state, :releases)
end

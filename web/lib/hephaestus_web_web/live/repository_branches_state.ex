defmodule HephaestusWebWeb.RepositoryBranchesState do
  @moduledoc "C2 state/effects contract for repository branch browsing."
  ...
  def reduce(state, event), do: RepositoryRouteModel.reduce(state, event, :branches)

  def execute(state, {:load, identity, generation}),
    do: RepositoryRouteModel.execute(state, identity, generation, :branches)

  def execute(effect, identity), do: RepositoryRouteModel.execute(effect, identity)

  def present(state), do: RepositoryRouteModel.present(state, :branches)
end

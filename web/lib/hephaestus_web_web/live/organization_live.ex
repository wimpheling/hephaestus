defmodule HephaestusWebWeb.OrganizationLive do
  ...
  def mount(_params, _session, socket) do
    ...
    if connected?(socket) do
      {state, [:load]} = OrganizationState.reduce(socket.assigns.page_state, :load)

      {:ok,
       socket
       |> assign(:page_state, state)
       |> PageStream.start_snapshot(OrganizationState)}
    else
      {:ok, socket}
    end
  end
  ...
  def handle_info({ref, event}, %{assigns: %{snapshot_task: %Task{ref: ref}}} = socket) do
    ...
    {state, _effects} = OrganizationState.reduce(socket.assigns.page_state, event)

defmodule HephaestusWebWeb.RepositoryCommitDetailState do
  @moduledoc "C2 state/effects contract for one immutable repository commit."
  ...
  def reduce(state, event), do: RepositoryRouteModel.reduce(state, event, :commit_detail)

  def execute(state, {:load, identity, generation}),
    do: RepositoryRouteModel.execute(state, identity, generation, :commit_detail)

  def execute(effect, identity), do: RepositoryRouteModel.execute(effect, identity)
  def present(state), do: RepositoryRouteModel.present(state, :commit_detail)
end

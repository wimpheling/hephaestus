defmodule HephaestusWebWeb.RepositoryFilesState do
  @moduledoc "C2 state/effects contract for repository file browsing."
  ...
  def reduce(state, event), do: RepositoryRouteModel.reduce(state, event, :files)

  def execute(state, {:load, identity, generation}),
    do: RepositoryRouteModel.execute(state, identity, generation, :files)

  def execute(effect, identity), do: RepositoryRouteModel.execute(effect, identity)

  def present(state), do: RepositoryRouteModel.present(state, :files)
end

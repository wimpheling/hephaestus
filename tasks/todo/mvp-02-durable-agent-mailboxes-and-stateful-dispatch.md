...
- [ ] **2. Persist mailboxes and events transactionally**
  ...
  - [ ] **Store bounded payloads safely**
    - [ ] Define inline and opaque external body thresholds, integrity hashes,
      quotas, retention, and cleanup.
    - [ ] Prevent producer-controlled paths, special files, content-type
      confusion, oversized decompression, and payload inclusion in NATS.
...
- [ ] **3. Dispatch eligible events**
  ...
  - [ ] **Use durable commands and inboxes**
    - [ ] Publish versioned start, retry, cancel, and recovery commands with
      stable IDs and bounded exact provenance.
    - [ ] Deduplicate every consumer so database retry and JetStream
      redelivery cannot launch duplicate logical runs.
    - [ ] Add tests for rollback, publisher retry, duplicate delivery, worker
      crash, and recovery at every authoritative transition.
...
- [ ] **5. Define retry, dead-letter, and recovery behavior**
  - [ ] **Handle outcomes honestly**
    - [ ] Define which provisioning, authorization, guest, protocol, timeout,
      ...
      operator recovery.
    - [ ] Use bounded exponential backoff, attempt limits, and explicit
      dead-letter disposition without losing the original event.
...
- [ ] **6. Add observability and operator inspection**
  ...
  - [ ] Add read-only inspection for an event's current disposition, attempts,
    denial reason, bound revision, active lease, and next recovery action.
...
latency measurements, and deliberate follow-up tasks.

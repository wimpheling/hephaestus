# MVP 06: Golden cooking-agent journey
...
release update, and exposes exact release, state, authorization, trigger,
capability-use, and result provenance.

This is the acceptance task for MVP 01 through MVP 05, not a source of new
...
- [`mvp-05-state-generations-and-run-provenance.md`](mvp-05-state-generations-and-run-provenance.md)
...
- [ ] **3. Install and bind the product slice**
  ...
  - [ ] Record the exact installation, revision, attachment, route,
    authorization snapshot, state generation, and secret binding fixture IDs.
...
- [ ] **6. Update and recover the stateful agent**
  ...
  - [ ] Execute the update hook in an isolated guest, activate the candidate,
    advance state generation, reopen the gate, and bind deferred events only
    to the new revision.
  ...
  - [ ] Recover the paused fixture through the authorized operator path and
    retain every historical revision, update, generation, event, and audit
    record.
...
- [ ] **7. Inspect exact provenance**
  - [ ] From the project UI or inspection API, resolve one journey from public
    ...
    cooking-agent revision, authorization snapshot, pre/post state
    generations, model and outbound uses, Git result, and final disposition.
...
and mailbox IDs, state generations, authorization snapshots, secret versions
and leases, model/outbound usage records, Git target/result commits, update and
recovery IDs, denial evidence, test counts, screenshots, and exact verification
commands.

...
The original cache object is missing; expiry is plausible from its lifecycle policy, but deletion or expiry was not proven. The resulting object has a new SHA-256 and does not claim to restore the original cache SHA. The replacement was uploaded under a new content-addressed object path with an immutable generation-0 precondition; the safe object metadata and same-generation SHA-256 roundtrip are recorded in [`upload-verification.json`](upload-verification.json). No overwrite or IAM change occurred. Three CI jobs passed for source SHA `d0f96f72adfceac53b95d5f6cb8dbaa3e452252e` in run `35675627526`; this is not a claim that a new documentation commit was tested. Fresh cache preflight, diagnostic, session-chat GCP acceptance, and merge approval remain pending because the designated cache path and cloud workload acceptance are separate steps.

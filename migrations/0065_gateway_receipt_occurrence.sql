-- Mediated mutations derive a method-scoped occurrence distinct from the
-- transport request ID. Gateway lifecycle receipts must follow that durable
-- occurrence so the RPC can reauthorize and return the committed receipt.
...
BEGIN
    change := CASE
        ...
        WHEN current_setting('hephaestus.gateway_install', true) = 'true' THEN 'updated'
        ELSE NULL
    END;

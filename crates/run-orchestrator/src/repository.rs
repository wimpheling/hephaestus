...
use crate::LIFECYCLE_EVENT_SUBJECT;
...
/// Transactional outbox record awaiting publication.
#[derive(Debug, Clone)]
...
pub trait RunRepository: Send + Sync + 'static {
    ...
    /// Loads unpublished outbox records.
    async fn unpublished_outbox(&self, limit: i64) -> Result<Vec<OutboxRecord>, RepositoryError>;
    /// Marks one publication as acknowledged by `JetStream`.
    async fn mark_outbox_published(&self, event_id: EventId) -> Result<(), RepositoryError>;

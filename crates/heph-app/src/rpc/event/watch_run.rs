...
use rpc_proto::messages::hephaestus::{
    common::v1::Cursor,
    event::v1::{WatchRunRequest, WatchRunResponse, watch_run_response},
};
use uuid::Uuid;

const AUDIENCE: &str = "/hephaestus.event.v1.ProductEventService/WatchRun";

pub(super) async fn handle(
    service: &EventRpc,
    ctx: RequestContext,
    request_message: ServiceRequest<'_, WatchRunRequest>,
    ...
    let id = request::required_id(message.run_id.as_option())
    ...
    let responses = stream::unfold(receiver, |mut receiver| async move {
        let result = receiver.recv().await?.map(|frame| WatchRunResponse {
            ...
            item: Some(match frame.delivery {
                Delivery::Barrier(value) => {
                    watch_run_response::Item::SnapshotBarrier(Box::new(value))
                }
                Delivery::Event(value) => watch_run_response::Item::Event(Box::new(value)),
                Delivery::Gap(value) => watch_run_response::Item::RetentionGap(Box::new(value)),
                Delivery::Revoked(value) => {
                    watch_run_response::Item::AccessRevoked(Box::new(value))
                }
            }),
            ..Default::default()
        });

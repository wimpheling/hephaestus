...
use rpc_proto::messages::hephaestus::{
    common::v1::Cursor,
    event::v1::{WatchProjectRequest, WatchProjectResponse, watch_project_response},
};
use uuid::Uuid;

const AUDIENCE: &str = "/hephaestus.event.v1.ProductEventService/WatchProject";

pub(super) async fn handle(
    service: &EventRpc,
    ctx: RequestContext,
    request_message: ServiceRequest<'_, WatchProjectRequest>,
    ...
    let id = request::required_id(message.project_id.as_option())
    ...
    let responses = stream::unfold(receiver, |mut receiver| async move {
        let result = receiver.recv().await?.map(|frame| WatchProjectResponse {
            ...
            item: Some(match frame.delivery {
                Delivery::Barrier(value) => {
                    watch_project_response::Item::SnapshotBarrier(Box::new(value))
                }
                Delivery::Event(value) => watch_project_response::Item::Event(Box::new(value)),
                Delivery::Gap(value) => watch_project_response::Item::RetentionGap(Box::new(value)),
                Delivery::Revoked(value) => {
                    watch_project_response::Item::AccessRevoked(Box::new(value))
                }
            }),
            ..Default::default()
        });

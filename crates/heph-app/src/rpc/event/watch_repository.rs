...
use rpc_proto::messages::hephaestus::{
    common::v1::Cursor,
    event::v1::{WatchRepositoryRequest, WatchRepositoryResponse, watch_repository_response},
};
use uuid::Uuid;

const AUDIENCE: &str = "/hephaestus.event.v1.ProductEventService/WatchRepository";

pub(super) async fn handle(
    service: &EventRpc,
    ctx: RequestContext,
    request_message: ServiceRequest<'_, WatchRepositoryRequest>,
    ...
    let responses = stream::unfold(receiver, |mut receiver| async move {
        let result = receiver.recv().await?.map(|frame| WatchRepositoryResponse {
            ...
            item: Some(match frame.delivery {
                Delivery::Barrier(value) => {
                    watch_repository_response::Item::SnapshotBarrier(Box::new(value))
                }
                Delivery::Event(value) => watch_repository_response::Item::Event(Box::new(value)),
                Delivery::Gap(value) => {
                    watch_repository_response::Item::RetentionGap(Box::new(value))
                }
                Delivery::Revoked(value) => {
                    watch_repository_response::Item::AccessRevoked(Box::new(value))
                }
            }),
            ..Default::default()
        });

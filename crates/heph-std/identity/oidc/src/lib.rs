...
use identity_application::{
    ExternalIdentityVerificationError, ExternalIdentityVerifier, VerifiedExternalIdentity,
};
...
impl ExternalIdentityVerifier for OidcVerifier {
    fn verify(
        ...
        Self::verify(self, token, expected_nonce).map_err(|_| ExternalIdentityVerificationError)
    }
}
...
mod tests {
    ...
    use identity_application::ExternalIdentityVerifier;
    ...
    #[test]
    fn provider_port_keeps_verification_failures_generic() {
        let verifier = OidcVerifier::new(
            "https://issuer.example",
            "hephaestus",
            Algorithm::HS256,
        ...
        let error = ExternalIdentityVerifier::verify(&verifier, "provider-secret", None)
            .expect_err("invalid token");
        assert_eq!(
            error.to_string(),
            "external identity token verification failed"
        );

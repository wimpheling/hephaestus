...
use secret_service::{
    ...
    ResolveRunSecrets, RotateSecret, SecretOutboxPublisher, SecretRuntimeService, SecretService,
    SecretServiceError,
};
...
async fn encrypted_rotation_delegation_revocation_and_purge_are_atomic() {
    ...
    let revoked_runtime_state: (i64, i64, bool, bool) = sqlx::query_as(
        "SELECT
             ...
             EXISTS(
                 ...
                 WHERE aggregate_id = $1
                   AND subject = 'hephaestus.secret.reconcile_revocation.v1'
             )",
    )
    .bind(secret_id.as_uuid())
    .fetch_one(&pool)
    ...
    assert_eq!(revoked_runtime_state, (0, 0, true, true));
...
#[tokio::test]
#[serial]
async fn secret_outbox_retry_is_deduplicated_by_jetstream() {
    ...
    sqlx::query(
        "UPDATE outbox SET published_at = now()
         WHERE aggregate_type = 'secret' AND published_at IS NULL",
    )
    .execute(&pool)
    .await
    .expect("isolate secret outbox fixture");
    let outbox_id = Uuid::new_v4();
    let subject = format!("hephaestus.secret.test.{}", outbox_id.simple());
    sqlx::query(
        "INSERT INTO outbox
         (id, aggregate_type, aggregate_id, subject, event_type, payload, occurred_at)
         VALUES ($1, 'secret', $2, $3, 'secret.test.v1',
    ...
    .execute(&pool)
    .await
    .expect("insert secret outbox fixture");

    let client = async_nats::connect(nats_url)
        .await
        .expect("NATS integration connection");
    let context = async_nats::jetstream::new(client);
    let stream_name = format!("HEPH_SECRET_TEST_{}", outbox_id.simple());
    let mut stream = context
        ...
        .await
        .expect("isolated secret stream");
    let publisher = SecretOutboxPublisher::new(context.clone(), pool.clone());
    assert_eq!(
        publisher
            .publish_pending(10)
            .await
    ...
    assert_eq!(stream.info().await.expect("stream state").state.messages, 1);
    ...
    sqlx::query("DELETE FROM outbox WHERE id = $1")
        ...
        .execute(&pool)
        .await
        .expect("clean secret outbox fixture");
}

...
/// Provider-neutral failure returned when an external token cannot be verified.
///
/// The error intentionally carries no provider detail or token material so it
/// can cross an authentication boundary without disclosing verification state.
#[derive(Debug, Clone, Copy, PartialEq, Eq, thiserror::Error)]
#[error("external identity token verification failed")]
pub struct ExternalIdentityVerificationError;

/// Verifies an external bearer or authorization-flow token at a provider port.
pub trait ExternalIdentityVerifier: Send + Sync {
    /// Verifies the token and returns only the provider-neutral verified identity.
    ///

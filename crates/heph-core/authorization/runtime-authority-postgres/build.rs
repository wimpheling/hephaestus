//! Ensures the runtime-authority `PostgreSQL` tests rebuild for every migration set.
...
fn main() {
    ...
    // Feed a deterministic manifest into rustc so this adapter always embeds
    // the exact schema it will apply.
    ...
    println!(
        "cargo::rustc-env=HEPHAESTUS_RUNTIME_AUTHORITY_MIGRATION_FINGERPRINT={fingerprint:016x}"
    );
}

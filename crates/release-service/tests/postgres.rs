...
use release_service::{
    ...
    RecoverInstanceUpdate, ReleaseArtifactInput, ReleaseOutboxPublisher, ReleaseService,
    RemoveAttachment, ReviseInstance, SetAttachmentEnabled, UpdateDecision, UpdateHookResult,
...
async fn publishes_once_and_imports_isolated_instances_with_exact_attachments() {
    ...
    assert!(matches!(
        stale_revision,
        Err(release_service::ReleaseServiceError::StaleInstanceRevision)
    ));
    ...
    assert!(matches!(
        broadened,
        Err(release_service::ReleaseServiceError::Domain(
    ...
    assert!(matches!(
        fork_update,
        Err(release_service::ReleaseServiceError::AgentFamilyMismatch)
    ));
    ...
    assert!(matches!(
        concurrent_update,
        Err(release_service::ReleaseServiceError::ConcurrentUpdate)
    ));
    ...
    assert!(matches!(
        drain_probe,
        Err(release_service::ReleaseServiceError::UpdateDrainPending)
    ));
    sqlx::query(
    ...
    let completed_update: serde_json::Value = sqlx::query_scalar(
        "SELECT payload FROM outbox
         WHERE aggregate_id = $1
           AND subject = 'hephaestus.agent_update.completed.v1'",
    )
    .bind(update_id.as_uuid())
    .fetch_one(&pool)
    .await
    .expect("durable completed update event");
    assert_eq!(completed_update["decision"], "activated");
    assert_eq!(
        completed_update["candidate_revision_id"],
        serde_json::Value::Null
    );
    assert_eq!(
        completed_update["revision_id"],
        update_candidate_revision.to_string()
    );
    ...
    let rejected_event: serde_json::Value = sqlx::query_scalar(
        "SELECT payload FROM outbox
         WHERE aggregate_id = $1
           AND subject = 'hephaestus.agent_update.rejected.v1'",
    )
    .bind(rejected_update_id.as_uuid())
    .fetch_one(&pool)
    .await
    .expect("durable update rejection event");
    assert_eq!(rejected_event["reason"], "hook_exit_nonzero");
    assert_eq!(rejected_event["exit_code"], 23);
    assert_eq!(
        rejected_event["message_id"],
        rejected_event["idempotency_key"]
    );
    ...
    let uncertain_events: Vec<(String, serde_json::Value)> = sqlx::query_as(
    ...
    .fetch_all(&pool)
    .await
    .expect("durable uncertain and paused events");
    assert_eq!(uncertain_events.len(), 2);
    assert_eq!(uncertain_events[0].0, "hephaestus.agent_instance.paused.v1");
    assert_eq!(uncertain_events[0].1["state"], "paused_unknown_state");
    assert_eq!(
        uncertain_events[1].0,
        "hephaestus.agent_update.uncertain.v1"
    );
    assert_eq!(uncertain_events[1].1["reason"], "hook_result_uncertain");
    ...
    assert!(tombstoned);
    let attachment_events: Vec<serde_json::Value> = sqlx::query_scalar(
        "SELECT payload FROM outbox
         WHERE aggregate_id = $1
           AND subject = 'hephaestus.agent_instance.attachment_changed.v1'
         ORDER BY occurred_at, id",
    )
    .bind(first_attachment.as_uuid())
    .fetch_all(&pool)
    .await
    .expect("transactional attachment events");
    assert_eq!(attachment_events.len(), 3);
    assert_eq!(attachment_events[0]["action"], "created");
    assert_eq!(attachment_events[1]["action"], "disabled");
    assert_eq!(attachment_events[2]["action"], "removed");
    service
        .revoke(
            &actor,
            key("revoke-release", update_release_id),
    ...
    let revocation_events: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM outbox
         WHERE aggregate_id = $1
           AND subject = 'hephaestus.release.revoked.v1'",
    )
    .bind(update_release_id)
    .fetch_one(&pool)
    .await
...
async fn release_outbox_retry_is_deduplicated_by_jetstream() {
    ...
    sqlx::query(
        "INSERT INTO outbox
         ...
         VALUES
         ($1, 'release', $2, 'hephaestus.release.published.v1',
          'release.published.v1', '{}', now()),

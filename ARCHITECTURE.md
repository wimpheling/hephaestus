...
| `ARCH-MAX-FILE-LENGTH` | Filesystem lint / migration-gated | Oversized hand-maintained Rust files signal mixed responsibilities; covers every repository-owned `.rs` under `crates/` and `examples/`, including untracked files, with a 350 physical-line limit. It excludes only the post-migration generated RPC subtree and Cooking vendor subtree. | `cargo dev check architecture` | Split cohesive modules or test responsibilities while preserving public paths; diagnostics name the path, count, limit, and remediation. |
| `DB-SQLX-ONLY-IN-POSTGRES-ADAPTERS` | Structural + lint / harness | Prevents persistence capability leaking inward; covers direct and transitive SQLx capability in the Cargo graph. Production adapters require `postgres_adapter = true`, valid `database_context`, and a `-postgres` package name; dev-only SQLx test fixtures require explicit `hephaestus.sqlx_test_dependency = true` metadata. | `cargo dev check architecture` | A `-postgres` suffix alone grants no capability; move SQL to a metadata-declared PostgreSQL adapter with the required name and context. |
...
| `DB-RLS-CONTEXT-REQUIRED` | SQL syntax lint + PostgreSQL integration / harness | Requires application-role queries to use a canonical actor transaction with actor, subject, request, and occurrence provenance; exact security-definer bootstrap resolvers precede actor discovery. | `cargo dev check architecture` | Use the scoped actor transaction or a declared, migration-backed bootstrap resolver; worker and migration pools stay explicit. |
...
| `DB-PAGINATION-STABLE-ORDER` | SQL contract lint + PostgreSQL integration / harness | Requires declared page queries to match stable `ORDER BY` keys, cursor predicates, and a unique final tie breaker; detects canonical undeclared or stale page contracts. | `cargo dev check architecture` | Add or correct the package-local `pagination.toml` contract and prove concurrent page traversal. |
...
| `RPC-DEADLINE-CANCELLATION-PROPAGATION` | Semantic lint / harness | Statically checks supported production RPC handlers and detached stream functions for one transport budget, approved helpers with the same budget argument, known direct adapter/receipt awaits, and syntactic send/receive/sleep waits. Runtime cancellation and deadline integration evidence remains required. | `cargo dev check architecture` | Use one `RequestBudget::from_transport`, an approved helper with `budget`/`&budget` in its declared position, and bounded producer operations. The checker does not prove arbitrary dynamic future taint, opaque helper internals, or runtime behavior. |
...
| Rule | Class / state | Rationale and scope | Command | Exceptions and remediation |
| --- | --- | --- | --- | --- |
| `EVT-STATE-AND-EVENT-COMMIT-ATOMICALLY` | SQL syntax lint + PostgreSQL integration / harness | Rejects direct production writes to durable event tables and requires explicit `append_application_event` calls to use a recognized transaction executor in the same Rust item; migration capture triggers cover authoritative state tables. | `cargo dev check architecture` | No direct event-table writes; append through the state transaction or install the committed capture trigger, then test rollback. |

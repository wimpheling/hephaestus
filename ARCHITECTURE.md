...
| `ARCH-MAX-FILE-LENGTH` | Filesystem lint / harness | Oversized hand-maintained Rust files signal mixed responsibilities; covers every repository-owned `.rs` under `crates/` and `examples/`, including untracked files, with a 350 physical-line limit. It excludes only the post-migration generated RPC subtree and Cooking vendor subtree. | `cargo dev check architecture` | Split cohesive modules or test responsibilities while preserving public paths; diagnostics name the path, count, limit, and remediation. |
| `DB-SQLX-ONLY-IN-POSTGRES-ADAPTERS` | Structural + lint / harness | Prevents persistence capability leaking inward; scans normal/build dependency paths transitively and separately validates dev-only SQLx edges. Production adapters require boolean `postgres_adapter = true`, a lowercase nonempty `database_context`, and a `-postgres` package name; dev-only SQLx fixtures require `sqlx_test_dependency = true`. Metadata regression coverage lives in `crates/heph-dev/src/checks/architecture/db_architecture_tests/metadata.rs`. | `cargo dev check architecture` | A `-postgres` suffix alone grants no capability; move SQL to a metadata-declared PostgreSQL adapter with the required name and context. |
...
| `DB-RLS-CONTEXT-REQUIRED` | SQL syntax lint + PostgreSQL integration / harness | Scans the six inventoried application-role pool bindings across five known adapter sources. It recognizes `begin_actor_transaction`/`begin_repeatable_read_actor_transaction`, requires verified context to set actor, subject, request, and occurrence settings, and permits only six exact migration-backed security-definer resolver forms. Direct pool queries, unscoped transactions, and pre-context queries fail; dynamic or opaque helper behavior is outside the scan. The two real disposable-PostgreSQL isolation tests are in `crates/heph-core/control-plane/postgres/tests/app_pool.rs`. | `cargo dev check architecture` | Use the scoped actor transaction or a declared, migration-backed bootstrap resolver; worker and migration pools stay explicit. |
...
| `DB-PAGINATION-STABLE-ORDER` | SQL contract lint + PostgreSQL integration / harness | Requires a package-local `pagination.toml` entry with a 1-based query index, declared order/cursor keys, uniform direction/operator, explicit unique tie-breaker and `unique_keys`, and one of `scalar`, `tuple`, `uuid_row_lookup`, or the pinned `stored_function` mode. The SQL `ORDER BY`, cursor predicate, and declaration must match exactly; stale declarations fail. Representative concurrent-write evidence passed in `crates/heph-core/gateway/postgres/tests/service_targets/uuid_pages.rs`, `crates/heph-core/gateway/postgres/tests/service_log_reader/snapshot.rs`, and `crates/heph-core/control-plane/postgres/tests/organization_pagination.rs`, covering UUID, snapshot, and composite-key traversal; remaining implementations continue to rely on the static contract until their own runtime paths are exercised. | `cargo dev check architecture` | Add or correct the package-local `pagination.toml` contract and prove concurrent page traversal. |
...
| `RPC-DEADLINE-CANCELLATION-PROPAGATION` | Semantic lint / harness | Scans production files under `crates/heph-app/src/rpc`, resolves supported service-method delegations, and requires exactly one `RequestBudget::from_transport`, an approved budget helper, and bounded syntactic send/receive/sleep waits for detached streams. It recognizes known adapter/receipt await markers and approved helper argument positions; dynamic future taint and opaque helper internals remain unproven. The disposable PostgreSQL/NATS artifact RPC test passed in `crates/heph-app/tests/artifact_deadline_cancellation.rs`: a raw H2 reset after response headers while the `release_artifacts` lock was held terminated the authorization query before lock release, and `SELECT 1` then proved pool reuse. Cancellation control opens a separate short-lived `PgConnection` only for `pg_cancel_backend(pid)` after the actor-scoped authorization future drops; that is an operational cancellation connection, not a product query, and is outside the RLS static rule. | `cargo dev check architecture` | Use one `RequestBudget::from_transport`, an approved helper with `budget`/`&budget` in its declared position, and bounded producer operations. The checker does not prove arbitrary dynamic future taint or opaque helper internals. |
...
| `SEC-SENSITIVE-NO-UNRESTRICTED-FORMAT` | Type structure + semantic lint / harness | Tracks the known request fields `handoff_secret`, `sid`, `value`, `token`, `credential`, `password`, `plaintext`, and `ciphertext` through local bindings, assignments, request-rooted field access, references, casts, and simple expressions. It rejects supported format/error/serialization sinks unless the value passes one of the recognized opaque ID conversions; interprocedural and dynamic flows remain outside the checker. Fixture coverage is in `crates/heph-dev/src/checks/architecture/rust_architecture/tests.rs`. | `cargo dev check architecture` | Exact redacted implementation only; remove derive or implement fixed redaction. |
| `SEC-NO-SENSITIVE-LOG-ARGUMENTS` | Semantic lint + sentinel integration / harness | Applies the same local-flow model to supported tracing/log macros, response/event/payload/error calls, metric-label methods, JSON/event macros, and sensitive output structs. Diagnostics report source/sink locations without values. The production UI installation RPC matrix passed success/failure non-disclosure coverage in `crates/heph-app/tests/ui_installation_rpc.rs`; no request-derived metric-label emission exists in current production paths, so the metric sink is guarded by static fixtures and a runtime metric assertion is currently inapplicable. | `cargo dev check architecture` | No plaintext exception; log opaque IDs or fixed classifications. |
...
| Rule | Class / state | Rationale and scope | Command | Exceptions and remediation |
| --- | --- | --- | --- | --- |
| `EVT-STATE-AND-EVENT-COMMIT-ATOMICALLY` | SQL syntax lint + PostgreSQL integration / harness | Checks the durable-event migration for `append_application_event`, capture functions/triggers, and the product outbox trigger. In production Rust items it rejects literal direct writes to `application_events`/`product_event_outbox` and requires `append_application_event` to use a recognized transaction executor in that same item; test modules are excluded. It cannot prove dynamic SQL, deployed trigger state, crash timing, or arbitrary transaction ownership. Eleven real disposable-PostgreSQL rollback, visibility, and schema/trigger tests passed under `crates/heph-app/tests/event_durability/`. | `cargo dev check architecture` | No direct event-table writes; append through the state transaction or install the committed capture trigger, then test rollback. |
| `EVT-OUTBOX-PUBLISHER-ONLY` | Source lint + NATS integration / harness | Restricts product-event publication to the designated `EventPublisher::publish_pending` adapter item after reading committed outbox rows. Legacy command outboxes keep their separate paths. | `cargo dev check architecture` | Worker composition may construct the publisher; publish product events only from committed outbox records. |

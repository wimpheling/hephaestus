...
| `RPC-DEADLINE-CANCELLATION-PROPAGATION` | Semantic lint / harness | Scans production files under `crates/heph-app/src/rpc`, resolves supported service-method delegations, and requires exactly one `RequestBudget::from_transport`, an approved budget helper, and bounded syntactic send/receive/sleep waits for detached streams. It recognizes known adapter/receipt await markers and approved helper argument positions; dynamic future taint, opaque helper internals, and runtime behavior remain unproven. Opt-in transport evidence is in `crates/heph-app/tests/artifact_deadline_cancellation.rs` and its `test-fixtures` implementation. | `cargo dev check architecture` | Use one `RequestBudget::from_transport`, an approved helper with `budget`/`&budget` in its declared position, and bounded producer operations. The checker does not prove arbitrary dynamic future taint, opaque helper internals, or runtime behavior. |
